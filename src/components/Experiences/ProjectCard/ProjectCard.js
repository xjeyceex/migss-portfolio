...
const Container = styled.div`
  max-width: 1200px;
  margin: 0 auto;
  padding: 0 1rem; /* Keep mobile padding at 1rem */
  ...
  @media(min-width:992px){ 
    padding: 0 80px; /* Extra space for larger buttons */
  }
`;
...
const AnimationContainer = styled.div`
  width: 100%;
  position: relative;
  overflow: hidden; /* Keep this to contain slide animations */
...
const NavButton = styled.button`
  ...
  justify-content: center;
  transition: .2s ease; 
  box-shadow: 0 4px 12px rgba(0,0,0,.15); /* Enhanced shadow */
  opacity: .95;
...
const Dot = styled.button`
  ...
  &:hover{ 
    ...
    transform: scale(${p=>p.active?"1.1":"1.2"}); 
  }
...
export default function ProjectCarousel() {
  ...
  const handleTouch = e => {
    ...
    if(Math.abs(diff) > 20) setHasInteracted(true);
      ...
      >
        ...
        <AnimationContainer>
          <AnimatePresence mode="wait">
            <motion.div
              key={current}
              initial={{opacity:0, x: slideDirection * 30}} // Reduced from 50 to 30
              animate={{opacity:1, x:0}}
              exit={{opacity:0, x: slideDirection * -30}} // Reduced from 50 to 30
              transition={{duration:.3,ease:"easeOut"}} // Reduced duration and changed easing
